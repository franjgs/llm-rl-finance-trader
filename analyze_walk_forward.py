...
class WalkForwardAnalyzer:
    ...
    def run(self) -> pd.DataFrame:
        ...
        if not results_df.empty:
            ...
            self.logger.info("=" * 90)
            self.logger.info("Ticker         : %s", self.ticker)
            self.logger.info("Interval       : %s", self.interval)
            self.logger.info("Train Start    : %s", self.train_start.date())
            self.logger.info("Final Date     : %s", self.final_date.date())
            self.logger.info("Step Days      : %d", self.step_days)
            self.logger.info("Total Steps    : %d", len(results_df))
            self.logger.info("-" * 90)
